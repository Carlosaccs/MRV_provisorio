...
async function carregarEstruturaSimulador() {
    try {
        ...
        console.warn("Aviso SpeedSim: Carregamento local.", e);
    }
}
...
function vincularEventosInterface() {
    ...
    if (btnAbrir && overlay) btnAbrir.addEventListener('click', () => overlay.style.display = 'flex');
    if (btnFechar && overlay) btnFechar.addEventListener('click', () => overlay.style.display = 'none');
